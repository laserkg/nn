...
def  TrainAda(df, feature_list, iteration, y,featureSampling=False, treeDepth=3):
    ...
    for k in range(iteration):
        ...
        cart = CART.TrainCART(trainData,subFeatures,y,treeDepth)
        trainData['pred'] = trainData[subFeatures].apply(lambda x: CART.predCART(x, cart), axis=1)
...
bank = 'bank.csv'
AllData = pd.read_csv(bank, header=0)
...
x1 = X_train.copy()
x2 = X_test.copy()
cart = CART.TrainCART(x1,Attributes,'y2',9)
x2['y3'] = x2[Attributes].apply(lambda x: CART.predCART(x, cart), axis=1)
...
T = 10
AdaModel = TrainAda(X_train, Attributes, T,'y2')
alpha = AdaModel['alpha']
cart_list = AdaModel['cart model']
pred = X_test[Attributes].apply(lambda x: CART.predCART(x, cart_list[0])*alpha[0], axis=1)
for k in range(1,T):
    pred2 = X_test[Attributes].apply(lambda x: CART.predCART(x, cart_list[k])*alpha[k], axis=1)
